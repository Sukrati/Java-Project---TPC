...
/**
 * Servlet implementation class TechCompanyServlet
 */
...
public class TechCompanyServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (toEdit.equals("visitDate"))
		{
			...
			if(k==1){
				int n=c.editTechCompany(toEdit, cIN, value);
				if(n==1)
				{
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		int k=dt.validDate(y,m,d);
		if(k==1){
		int n=c.insertTechCompany(cIN, cname, emailID, cpi, city, visitdate, salary, requirements, branch);
		if(n==1)
		{
